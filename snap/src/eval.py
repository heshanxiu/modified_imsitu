...
def eval_model(dataset, dataset_loader, standard_encoding, model_encoding, model, trustedEncoder=False, image_group={}):
    ...
    if trustedEncoder == False:
        print(
            "not using trusted encoder. This may take signficantly longer as predictions are converted to other encoding.")
    mx = len(dataset_loader)
    batches = []
    top1 = imSituTensorEvaluation(1, 3, image_group)
    top5 = imSituTensorEvaluation(5, 3, image_group)
    for i, (indexes, input, target) in tqdm(enumerate(dataset_loader)):
        if i > 3900:
            break
        if args.extract_feature is True and input is torch.zeros(36, 768):
            continue
        # if True or i % 10 == 0:
        #     print("batch {} out of {}\r".format(i + 1, mx)),
...
def eval_file(output, dataset, standard_encoding, image_group):
    ...
    for line in tqdm(f):
        ...
        if curr_image == "":
            ...
            encoded_predictions = standard_encoding.to_tensor(predictions, False, False).view(1,
                                                                                              standard_encoding.n_verbs(),
                                                                                              -1)
    ...
    # last one
    predictions = sorted(predictions, key=lambda x: standard_encoding.v_id[x["verb"]])
    encoded_predictions = standard_encoding.to_tensor(predictions, False, False).view(1, standard_encoding.n_verbs(),
                                                                                      -1)
    indexes = torch.LongTensor(order).view(1, -1)
    encoded_reference = standard_encoding.to_tensor([dataset[curr_image]]).view(1, -1)
    top1.add_point(encoded_reference, encoded_predictions, indexes, [curr_image])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sparsity_min", default=0, type=int,
                        help="only evaluate images that meet some sparsity threshold")
    parser.add_argument("--sparsity_max", default=-1, type=int,
                        help="only evaluate images that meet some sparsity threshold")
    ...
    parser.add_argument("--trust_encoder", default=False,
                        help="if trusted, skip re-encoding intro trusted, which takes a long time", action='store_true')
    parser.add_argument("--feature_path", default="/local/shanxiu/lxmert/snap/visualsrl/visualsrl_extract_dev_15656/lxmert_feature", help="location of feature h5py path")
    parser.add_argument("--img_index_path",
                        default="/local/shanxiu/lxmert/snap/visualsrl/visualsrl_extract_dev_15656/image_id_to_index.json",
                        help="location of img index to h5py file")
    parser.add_argument("--extract_feature",
                        default=False,
                        help="extract feature or not")
    parser.add_argument("--feature_level", default=-1, help="extract which level of feature")
    ...
    # compute sparsity statistics （from train)
    verb_role_noun_freq = {}
    for image, frames in train_set.items():
        # for each image in train
        v = frames["verb"]
        items = set()
        # Q1 this means that for each image, even though there is a pair (v, r, n) that appears
        # more than once, but only counted as one towards to sum?
        for frame in frames["frames"]:
            # for each image possible frames to one verb
            for (r, n) in frame.items():
                # for each (r, n) pair in one element of frame list
                key = (v, r, n)
                items.add(key)
        for key in items:
            if key not in verb_role_noun_freq:
                verb_role_noun_freq[key] = 0
            verb_role_noun_freq[key] += 1
    # per role it is the most frequent prediction: every combination of v, n(v, r, n)
    # and among roles its the most rare: iterate through all r, find the most rare as f

    with open(args.dataset_dir + "/" + args.eval_file, "r") as eva:
        eval_dataset = json.load(eva)
    image_sparsity = {}
        ...
        for frame in frames["frames"]:
            for (r, n) in frame.items():
                ...
                if key not in verb_role_noun_freq:
                    ...
                    freq = verb_role_noun_freq[key]
                if r not in role_max or role_max[r] < freq:
                    role_max[r] = freq
                # iterate through image, image's possible frame, to find maximum (v, r, n) pair
                # for each semantic role
        min_val = -1
        for (r, f) in role_max.items():
            if min_val == -1 or f < min_val:
                min_val = f
        image_sparsity[image] = min_val
        # for each image, find each role's most frequent (v, r, n)
        # then fine the most rare role for each image into image_sparsity

    if args.sparsity_max > -1:
        x = range(args.sparsity_min, args.sparsity_max + 1)
        print("evaluating images where most rare verb-role-noun in training is x , s.t. {} <= x <= {}".format(
            args.sparsity_min, args.sparsity_max))
        n = 0
    ...
    if args.format == "model":
        ...
        # print(model_module)
        # modify
        # model_loader = importlib.machinery.SourceFileLoader(mod_name, args.include)
        # model_module = model_loader.load_module()
        ...
        if args.weights_file is None:
            print("expecting weight file to run features/ or not need weights?")
            # exit()
        else:
        ...
        if args.trust_encoder:
            dataset_encoder = encoder

        dataset = imSituSituation(args.image_dir, eval_dataset, dataset_encoder,
                                  args.feature_level,
                                  model.dev_preprocess(),
                                  args.feature_path, args.img_index_path, args)
    ...
    print(
        "\ntop-1\n\tverb     \t{:.2f}%\n\tvalue    \t{:.2f}%\n\tvalue-all\t{:.2f}%\ntop-5\n\tverb     \t{:.2f}%\n\tvalue    \t{:.2f}%\n\tvalue-all\t{:.2f}%\ngold verbs\n\tvalue    \t{:.2f}%\n\tvalue-all\t{:.2f}%\nsummary \n\tmean    \t{:.2f}%".format(
            100 * top1_a["verb"], 100 * top1_a["value"], 100 * top1_a["value-all"], 100 * top5_a["verb"],
            100 * top5_a["value"], 100 * top5_a["value-all"], 100 * top5_a["value*"], 100 * top5_a["value-all*"],
            100 * avg_score))
