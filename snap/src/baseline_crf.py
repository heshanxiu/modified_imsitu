...
class vgg_modified(nn.Module):
    ...
    def forward(self, x):
        return self.dropout2(
            self.relu2(self.lin2(self.dropout1(self.relu1(self.lin1(self.vgg_features(x).view(-1, 512 * 7 * 7)))))))
...
class baseline_crf(nn.Module):
    ...
    def forward(self, image):
        ...
        # if args.extract_feature is True:
        rep = image.mean(1)
        # else:
        #     rep = self.cnn(image)
        # print self.rep_size
...
def train_model(max_epoch, eval_frequency, train_loader, dev_loader, model, encoding, optimizer, save_dir,
                timing=False):
    model.train()
    ...
    for k in range(0, max_epoch):
        for i, (index, input, target) in enumerate(train_loader):
            ...
            if total_steps % print_freq == 0:
                ...
                print("{},{},{}, {} , {}, loss = {:.2f}, avg loss = {:.2f}, batch time = {:.2f}".format(total_steps - 1,
                                                                                                        k, i,
                                                                                                        format_dict(
                                                                                                            top1_a,
                                                                                                            "{:.2f}",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--command", choices=["train", "eval", "predict", "features", "train_on_dev", "eval_on_dev"], required=True)
    ...
    parser.add_argument("--feature_path",
                        default="/local/shanxiu/lxmert/snap/visualsrl/visualsrl_extract_dev_15656/lxmert_feature",
                        help="location of feature h5py path")
    parser.add_argument("--img_index_path",
                        default="/local/shanxiu/lxmert/imSitu/visualsrl_data_dev",
                        help="location of img index to h5py file")
    parser.add_argument("--extract_feature",
                        default=False,
                        help="extract feature or not")
    parser.add_argument("--feature_level", default=-1,
                        help="extract which level of feature")

    args = parser.parse_args()
    if args.command == "train_on_dev":
        print("command = train on dev")

        with open("./dev.json", "r") as train:
            train_set = json.load(train)
        with open("./dev.json", "r") as dev:
            dev_set = json.load(dev)
        ...
        train_to_index = os.path.join(args.img_index_path, "train.json")
        dev_to_index = os.path.join(args.img_index_path, "dev.json")
        dataset_train = imSituSituation(args.image_dir, train_set, encoder, args.feature_level,
                                        model.train_preprocess(),
                                        args.feature_path, train_to_index)
        dataset_dev = imSituSituation(args.image_dir, dev_set, encoder, args.feature_level,
                                      model.dev_preprocess(),
                                      args.feature_path, dev_to_index)
    ...
    if args.command == "train":
        ...
        dataset_train = imSituSituation(args.image_dir, train_set, encoder, args.feature_level,
                                        model.train_preprocess(),
                                        args.feature_path, args.img_index_path)
        dataset_dev = imSituSituation(args.image_dir, dev_set, encoder, args.feature_level,
                                      model.dev_preprocess(),
                                      args.feature_path, args.img_index_path)
        ...
        dataset = imSituSituation(args.image_dir, eval_file, encoder, args.feature_level,
                                  model.dev_preprocess(),
                                  args.feature_path, args.img_index_path)
        ...
        image_loader = torch.utils.data.DataLoader(folder_dataset, batch_size=args.batch_size, shuffle=False,
                                                   num_workers=3)
