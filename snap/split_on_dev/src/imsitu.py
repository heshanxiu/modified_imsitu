    ...
    def add_point(self, encoded_reference, encoded_predictions, sorted_idx, image_names=None):
        ...
        for i in range(0, b):
            ...
            if sc_key not in self.score_cards:
                new_card = {"verb": 0.0, "n_image": 0.0, "value": 0.0, "value*": 0.0, "n_value": 0.0, "value-all": 0.0,
                            "value-all*": 0.0}
        ...
        for (key, card) in self.score_cards.items():
            ...
            if len(groups) == 0 or g in groups:
                if v not in agg_cards:
                    new_card = {"verb": 0.0, "n_image": 0.0, "value": 0.0, "value*": 0.0, "n_value": 0.0,
...
class imSituVerbRoleNounEncoder:
    ...
    def n_roles(self):
        return len(self.r_id)

    def verbposition_role(self, v, i):
        return self.v_r[v][i]

    def verb_nroles(self, v):
        return len(self.v_r[v])
    ...
    def __init__(self, dataset):
        self.v_id = {}
        # iterate through .item to get all verb -> assign id according to length
        self.id_v = {}

        self.r_id = {}
        # mark all semantic roles
        self.id_r = {}
        ...
        self.mr = 0
        # maximum frequency of appearances of a (v, r) pair

        self.v_r = {}
        # key (_id) length of verb dictionary
        # value [r1, r2, r3, ...] for each verb

        for (image, annotation) in dataset.items():
            # json.load(train.json)
            v = annotation["verb"]
...
class imSituVerbRoleLocalNounEncoder(imSituVerbRoleNounEncoder):
    ...
    def n_verbrole(self):
        return len(self.vr_id)

    def n_verbrolenoun(self):
        return self.total_vrn

    def verbposition_role(self, v, i):
        return self.v_vr[v][i]

    def verb_nroles(self, v):
        return len(self.v_vr[v])
...
class imSituSituation(data.Dataset):
    def __init__(self, root, annotation_file, encoder, feature_level,transform=None, feat_path=None, img_index_path=None):
        ...
        self.img_to_index_path = img_index_path
        self.img_index = None
    ...
    def __getitem__(self, index):
        ...
        if img_ind != -1 and self.level != -1:
            with h5py.File(self.feature_path, "r") as f:
                img = f['features'][img_ind, int(self.level)-1]
                img = torch.from_numpy(img)
        elif img_ind == -1:
            img = torch.zeros(36, 768)
        else:
